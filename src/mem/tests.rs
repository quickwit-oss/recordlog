...
fn test_mem_queues_already_exists() {
    ...
    mem_queues
        .create_queue("droopy", FileNumber::for_test(1u32))
        .unwrap();
    assert!(matches!(
        mem_queues.create_queue("droopy", FileNumber::for_test(1u32)),
        Err(AlreadyExists)
    ));
}
...
fn test_mem_queues() {
    let mut mem_queues = MemQueues::default();
    mem_queues
        .create_queue("droopy", FileNumber::for_test(1))
        .unwrap();
    mem_queues
        .create_queue("fable", FileNumber::for_test(1))
        .unwrap();
    {
        assert!(mem_queues
...
fn test_mem_queues_truncate() {
    let mut mem_queues = MemQueues::default();
    mem_queues.create_queue("droopy", 1u32.into()).unwrap();
    {
        assert!(mem_queues
...
fn test_mem_queues_skip_yield_error() {
    let mut mem_queues = MemQueues::default();
    mem_queues
        .create_queue("droopy", FileNumber::from(1u32))
        .unwrap();
    assert!(mem_queues
        .append_record("droopy", 1.into(), 0, b"hello")
        .is_ok());
...
fn test_mem_queues_append_in_the_past_yield_error() {
    let mut mem_queues = MemQueues::default();
    mem_queues.create_queue("droopy", 1.into()).unwrap();
    assert!(mem_queues
        .append_record("droopy", 1.into(), 0, b"hello")
        .is_ok());
    assert!(mem_queues
